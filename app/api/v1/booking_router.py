...
async def reserve(req: BookingRequest, user = Depends(get_current_user)):
    ...
    booking_ids = await BookingService.book_seats(user["id"], req.trip_id, req.seat_numbers, req.pay_with_wallet)
    return {"booking_ids": booking_ids}

@router.post("/cancel")
async def cancel_bookings(req: CancelRequest, current_user=Depends(get_current_user)):

    result = await BookingService.cancel_bookings(current_user["id"], req.booking_ids)
    return result
